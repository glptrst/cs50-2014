...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            //variables for loops 
            int i, j, a;
            //there should be 2 blanks in the request line
            ...
            if (blanks_counter != 2)
                error(400);
            //there should be 1 substring "\r\n"
            if ( ( strstr(line, "\r\n") ) == NULL )
                error(400);            
            char method[strlen(line)];
            char target[strlen(line)];
            char version[strlen(line)]; 
            //isolate method
            i = 0;
            j = 0;
            while (true)
            ...
            //isolate version
            i = 0;
            j = strlen(method) + 1 + strlen(target) + 1;
            while (true)
            {
                if (line[j] == '\r')
                {
            ...
            if ( strchr(target, 46) == NULL )
                error(501);

            // TODO: extract query from request-target
            char query[strlen(line)];
            //if there is no question mark 
            if ( strchr(target, 63) == NULL )
                query[0] = '\0';
            else
            {
                //find question mark index (beginning of query)
                a = 0;
                while ( target[a] != 63 )
                    a++;      
                //if there is just the ?
                if (target[a+1] == '\0')
                    query[0] = '\0';
                //else isolate query
                else
            ...
            char path[strlen(line) + strlen(root)];   
            char absolute_path[ strlen(line) ];    
            //create absolute path
            //if there is a query
            if ( strlen(query) > 0 )
            {
                //copy target in absolute_path till the ? (excluded)
                i = 0;
                while (true)
                {
                    if (target[i] == '?')
                    ...
                    absolute_path[i] = target[i];            
                    i++;
                }
            }
            //if there is no query absolute_path equals to target
            else
            {   //if there is the question mark
                if ( strchr(target, 63) != NULL )
                {
                    i = 0;
                    while (true)
                ...
                else //if there is no question mark
                {
                    i = 0;
                    while (true)
                    {
            ...
            //create path (concatenation of root and absolute path)
            char root1[100];
            strcpy(root1, root);
            strcpy( path, strcat(root1, absolute_path) );
            ...
            char extension[ strlen(absolute_path) ];
            //find index of '.' in absolute_path
            for ( j = 0 ; absolute_path[j] != '.'; j++ )
            ;
            j++;   
            //create extension
            i = 0;
            while (true)
            {
                if (absolute_path[j] == '\0')
                {
                    extension[i] = '\0';
                    break;
                }      
                extension[i] = absolute_path[j];
                i++;
                j++;
            }
